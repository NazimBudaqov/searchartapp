...
from ...models import MainData, Country, Indicator
...
class MainDataImportViewSet(ViewSet):
    def post(self, csv_data):
        ...
        indicators = {}
        countries = {}
        batch_size = 620  # faylda ~10000 rows eleyir

        for _, row in csv_data.iterrows():
            subsec_name = row["Subsector"]
            ...
            # Create or retrieve Indicator and Country objects
            if indicator_name not in indicators:
                try:
                    ...
                    indicator = Indicator.objects.create(
                        ...
                        subsector=Subsector.objects.get(subsectorName=subsec_name),
                    )
            ...
            if country_name not in countries:
                try:
                    ...
                    country = Country.objects.create(countryName=country_name)
            ...
            print(
                f"{_}. Year: {year} Indicator: {indicator_name} Country: {country_name} "
            )

        print(
            f"batch size is: {batch_size} from total lenght of {len(data_dict)} \nStarting bulk_creating of objects..."
        )
        ...
        for (indicator_name, country_name), data in data_dict.items():
            ...
            maindata_list.append(
                MainData(indicator=indicator, country=country, json_data=data)
            )

            # Perform bulk_create in batches
            if len(maindata_list) >= batch_size:
                MainData.objects.bulk_create(maindata_list)
                maindata_list = []

        # Insert any remaining data in the maindata_list
        if maindata_list:
            MainData.objects.bulk_create(maindata_list)

        return {"main data length": MainData.objects.count()}

    # def populate(self, data_dict, indicators, countries):
    #     maindata_list = []
    #     for (indicator_name, country_name), data in data_dict.items():
    #         indicator = indicators[indicator_name]
    #         country = countries[country_name]
